...
export class AppendingLineChart {
    ...
    constructor(container, lineColors: string[]) {
        ...
        this.xScale = scaleLinear().domain([0, 0]).range([0, width]);

        this.yScale = scaleLinear().domain([0, 0]).range([height, 0]);
        ...
        for (let i = 0; i < this.numLines; i++) {
            this.paths[i] = this.svg.append("path").attr("class", "line").styles({
                fill: "none",
                stroke: lineColors[i],
                "stroke-width": "1.5px",
            });
