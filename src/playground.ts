...
import { axisBottom, linkHorizontal, scaleLinear, scalePoint } from "d3";
...
class Player {
    ...
    private callback: null | ((isPlaying: boolean) => void) = null;
...
let linkWidthScale = scaleLinear().domain([0, 5]).range([1, 10]).clamp(true);
...
function makeGUI() {
    ...
    let x = scaleLinear().domain([-1, 1]).range([0, 144]);
    let xAxis = axisBottom(x).tickValues([-1, 0, 1]).tickFormat(d3.format("d"));
...
function drawNode(
    ...
    let nodeGroup = container.append("g").attrs({
        class: "node",
        id: `node${nodeId}`,
...
function drawNetwork(network: nn.Node[][]): void {
    ...
    let layerScale = scalePoint(d3.range(1, numLayers - 1), [featureWidth, width - RECT_SIZE]);
...
function updateHoverCard(
    ...
    let value =
        type === HoverType.WEIGHT ? (nodeOrLink as nn.Link).weight : (nodeOrLink as nn.Node).bias;
...
function drawLink(
    ...
    container
        ...
        .on("mouseenter", function () {
            updateHoverCard(HoverType.WEIGHT, input, d3.pointer(this));
        })
        .on("mouseleave", function () {
            updateHoverCard(null);
        });
...
function drawDatasetThumbnails() {
    ...
    if (state.problem === Problem.REGRESSION) {
        for (let regDataset in regDatasets) {
            let canvas: any = document.querySelector(`canvas[data-regDataset=${regDataset}]`);
...
function hideControls() {
    ...
    HIDABLE_CONTROLS.forEach(([text, id]) => {
        ...
        let input = label.append("input").attrs({
            type: "checkbox",
...
function generateData(firstTime = false) {
    ...
    let numSamples =
        state.problem === Problem.REGRESSION ? NUM_SAMPLES_REGRESS : NUM_SAMPLES_CLASSIFY;
    let generator = state.problem === Problem.CLASSIFICATION ? state.dataset : state.regDataset;
