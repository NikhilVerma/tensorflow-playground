...
import { axisBottom, axisRight, range, rgb, scaleLinear, scaleQuantize, Selection } from "d3";
...
export class HeatMap {
    ...
    constructor(
        ...
        let tmpScale = scaleLinear<string, number>()
            .domain([0, 0.5, 1])
            .range(["#f59322", "#e8eaeb", "#0877bd"])
            .clamp(true);
        // Due to numerical error, we need to specify
        // range(0, end + small_epsilon, step)
        // in order to guarantee that we will have end/step entries with
        // the last element being equal to end.
        let colors = range(0, 1 + 1e-9, 1 / NUM_SHADES).map((a) => {
            return tmpScale(a);
        });
        this.color = scaleQuantize().domain([-1, 1]).range(colors);

        container = container.append("div").styles({
            width: `${width}px`,
            height: `${height}px`,
            position: "relative",
            top: `-${padding}px`,
